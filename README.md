...
| [Spring Boot Advanced Features](examples/spring-boot-advanced-features) | Eight self-contained feature demos targeting the cases MapStruct cannot match (see below) |

### Advanced Features example — what each package shows

| Package | Endpoint | Feature |
|---|---|---|
| `strictmapping/` | `GET /reports` | Unmapped target component is a **build error** by default, not a silent `null` |
...
| `sealed/` | `GET /notifications` | Sealed interface mapped exhaustively — one method per permitted record subtype, no default branch |
